...
inline __device__ half2 GetRoPEres(half data, half data_rotate, const float2 coef)
{
    float f_data1 = __half2float(data);
    float f_data2 = __half2float(data_rotate);
    float2 rot_fv = GetRoPEres(f_data1, f_data2, coef);
                                                   ...
                                                   bool use_dynamic_ntk /*placeholder for ntk RoPE*/)
{
    ...
    // v does not in RoPE's scope, so store it to v_buf directly
    float v = QKV[v_id];
